...
import { Camera, Film, Navigation as PingPong, ArrowRight, Plus, Star, X } from 'lucide-react';
...
export default function HobbiesHome() {
  const { photography, videoEditing, tableTennis, customHobbies, addCustomHobby, deleteCustomHobby } = useHobbiesStore();
  const [showAdd, setShowAdd] = useState(false);
  const [newHobby, setNewHobby] = useState({ name: '', description: '', icon: 'Star', color: '#8b5cf6' });

  const subPages = [
    { path: '/hobbies/photography', icon: Camera, title: 'Photography', desc: 'Gears, shoots, portfolio', color: '#f59e0b' },
    { path: '/hobbies/video-editing', icon: Film, title: 'Video Editing', desc: 'Projects and techniques', color: '#3b82f6' },
    { path: '/hobbies/table-tennis', icon: PingPong, title: 'Table Tennis', desc: 'Match scores and skills', color: '#ef4444' },
  ];
  ...
  return (
    <PageWrapper>
      <div className="page-header">
        ...
        <p>Creative pursuits and sports tracking</p>
      ...
      <div className="grid-3" style={{ marginBottom: 'var(--space-xl)' }}>
        <StatsCard icon={Camera} label="Photo Shoots" value={photography.length} subtitle="Albums/Projects" color="#f59e0b" />
        <StatsCard icon={Film} label="Videos Edited" value={videoEditing.filter(v => v.status === 'completed').length} subtitle="Published" color="#3b82f6" delay={0.1} />
        <StatsCard icon={PingPong} label="TT Matches" value={tableTennis.length} subtitle="Games logged" color="#ef4444" delay={0.2} />
      </div>
      ...
      <div className="grid-auto">
        ...
        {(customHobbies || []).map((hobby, i) => {
          ...
          return (
            <motion.div key={hobby.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: (subPages.length + i) * 0.1 }}>
