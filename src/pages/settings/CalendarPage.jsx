...
import { Calendar, ChevronLeft, ChevronRight, RefreshCw, ExternalLink, LogIn, Clock, ListTodo, CheckCircle2, Circle } from 'lucide-react';
    ...
    return (
      <PageWrapper>
        <div className="page-header">
          ...
          <p>View your Google Calendar events and Tasks in one place</p>
        </div>
        <motion.div className="glass-card" style={{ padding: '3rem', textAlign: 'center' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p style={{ color: 'var(--text-tertiary)', marginBottom: '1.5rem', maxWidth: 400, margin: '0 auto 1.5rem' }}>
            Sign in with Google to view your calendar events and tasks right here in Second Brain OS.
          </p>
    ...
    <PageWrapper>
      ...
      <div className="tab-nav" style={{ marginBottom: 'var(--space-lg)' }}>
        <button className={activeTab === 'calendar' ? 'active' : ''} onClick={() => setActiveTab('calendar')}>
          <Calendar size={14} style={{ marginRight: 4 }} /> Calendar
        </button>
        <button className={activeTab === 'tasks' ? 'active' : ''} onClick={() => setActiveTab('tasks')}>
          <ListTodo size={14} style={{ marginRight: 4 }} /> Google Tasks ({pendingGTasks.length})
        </button>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 'var(--space-lg)' }}>
          ...
          <motion.div className="glass-card" style={{ overflow: 'hidden' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div style={{
              ...
              padding: '1rem 1.25rem', borderBottom: '2px solid var(--border-primary)',
            }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <button onClick={goToday} style={{
                  background: 'none', border: 'none', color: 'var(--accent-purple-light)',
                  fontSize: 'var(--font-xs)', cursor: 'pointer', fontWeight: 600,
                }}>Today</button>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--border-primary)' }}>
              {DAYS.map(d => (
                <div key={d} style={{
                  padding: '0.5rem', textAlign: 'center', fontSize: 'var(--font-xs)',
              ...
              {selectedEvents.length === 0 ? (
                ...
                <div style={{ maxHeight: 500, overflowY: 'auto' }}>
                  {selectedEvents.map(ev => (
                    <div key={ev.id} style={{
                      padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--border-primary)',
                      display: 'flex', gap: '0.75rem', alignItems: 'flex-start',
                    }}>
                      <div style={{
                        width: 4, minHeight: 36, borderRadius: 2, flexShrink: 0, marginTop: 2,
                        background: EVENT_COLORS[ev.color] || EVENT_COLORS.default,
                      }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {ev.description && (
                          <div style={{ fontSize: 'var(--font-xs)', color: 'var(--text-muted)', marginTop: 4, lineHeight: 1.4, maxHeight: 40, overflow: 'hidden' }}>
                            {ev.description.substring(0, 100)}
                          </div>
                        )}
                      </div>
                      {ev.htmlLink && (
                        <a href={ev.htmlLink} target="_blank" rel="noreferrer" className="btn-icon" title="Open in Google Calendar">
                ...
                {pendingGTasks.map(task => (
                  <div key={task.id} style={{
                    ...
                    <input
                      type="checkbox"
                      checked={false}
                      onChange={() => toggleGoogleTaskComplete(task.listId, task.id, false)}
                      style={{ width: 18, height: 18, cursor: 'pointer', accentColor: '#8b5cf6', flexShrink: 0, marginTop: 2 }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.4rem', marginTop: 4, alignItems: 'center' }}>
                        ...
                        {task.due && (
                          <span style={{
                            ...
                            color: new Date(task.due) < new Date() ? 'var(--accent-red)' : 'var(--text-tertiary)',
                          }}>
            ...
            {completedGTasks.length === 0 ? (
              ...
              <div style={{ maxHeight: 500, overflowY: 'auto' }}>
                {completedGTasks.slice(0, 20).map(task => (
                  <div key={task.id} style={{
                    padding: '0.6rem 1.25rem', borderBottom: '1px solid var(--border-primary)',
                    display: 'flex', alignItems: 'center', gap: '0.75rem', opacity: 0.6,
                  }}>
                    <input
                      type="checkbox"
                      checked={true}
                      onChange={() => toggleGoogleTaskComplete(task.listId, task.id, true)}
                      style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#10b981', flexShrink: 0 }}
                    />
                    <span style={{ fontSize: 'var(--font-sm)', textDecoration: 'line-through', color: 'var(--text-tertiary)' }}>{task.title}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Last synced */}
      {lastFetched && (
        <div style={{ textAlign: 'center', fontSize: 'var(--font-xs)', color: 'var(--text-muted)', marginTop: 'var(--space-lg)' }}>
          Last synced: {new Date(lastFetched).toLocaleTimeString('en-IN')}
        </div>
      )}
    </PageWrapper>
  );
